...
import type { ClimbSegment, DescentSegment } from "@/lib/types";
...
export default function RoutesPage() {
  ...
  const [activeDescentIndex, setActiveDescentIndex] = useState<number | null>(null);
              ...
              <CardBody className="pt-4">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-text-muted">
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    {r.downhill_profile?.length || 0} Abfahrten
                  </span>
                </div>

                {r.climb_profile.length > 0 && (
                  <ul className="mt-3 space-y-1 border-t border-border/40 pt-2">
                    {r.climb_profile.slice(0, 2).map((c: ClimbSegment, i: number) => (
                      <li key={`c-${i}`} className="flex items-center justify-between text-xs text-text-muted">
                        <span className="tabular-nums">
                          km {c.start_km}–{c.end_km} ({(c.length_m / 1000).toFixed(1)} km)
                        </span>
                        <div className="flex items-center gap-2">
                          <span className="tabular-nums font-medium text-text">+{Math.round(c.elevation_gain_m)} hm (Ø {c.avg_grade_pct}%)</span>
                ...
                {r.downhill_profile && r.downhill_profile.length > 0 && (
                  <ul className="mt-2 space-y-1 border-t border-border/30 pt-1.5">
                    {r.downhill_profile.slice(0, 2).map((d: DescentSegment, i: number) => (
                      <li key={`d-${i}`} className="flex items-center justify-between text-xs text-text-muted">
                        <span className="tabular-nums">
                          km {d.start_km}–{d.end_km} ({(d.length_m / 1000).toFixed(1)} km)
                        </span>
                        <div className="flex items-center gap-2">
                          <span className="tabular-nums font-medium text-blue-400">-{Math.round(d.elevation_loss_m)} hm (Ø {d.avg_grade_pct}%)</span>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-text-muted uppercase tracking-wider font-semibold">
                    {selectedRoute.sport} · {formatDistance(selectedRoute.distance_m)} · +{Math.round(selectedRoute.elevation_gain_m)} hm / -{Math.round(selectedRoute.elevation_loss_m || 0)} hm
                  </p>
                </div>
              </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-text-muted mb-2 flex items-center gap-1.5">
                  ...
                  Interaktive Topografische Karte & Segmente
                </h3>
                <RouteMap
                  trackPoints={selectedRoute.analysis?.track_points || []}
                  climbs={selectedRoute.climb_profile}
                  downhills={selectedRoute.downhill_profile || []}
                  activeClimbIndex={activeClimbIndex}
                  activeDescentIndex={activeDescentIndex}
                  hoveredPoint={hoveredPoint}
                  onClimbClick={(idx) => {
                    setActiveClimbIndex(idx);
                    setActiveDescentIndex(null);
                  }}
                  onDescentClick={(idx) => {
                    setActiveDescentIndex(idx);
                    setActiveClimbIndex(null);
                  }}
                />
              </div>

              {/* Elevation Profile Chart with Marked Climb and Downhill Segments */}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="bg-surface p-3 rounded-xl border border-border/60">
                  <span className="text-[10px] uppercase font-bold text-text-muted flex items-center gap-1">
                    <Flame className="h-3 w-3 text-amber-500" /> Anstiege & Abfahrten
                  </span>
                  <p className="text-base font-bold text-text mt-0.5">
                    {selectedRoute.climb_profile.length} Anst. / {selectedRoute.downhill_profile?.length || 0} Abf.
                  </p>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-text-muted flex items-center gap-1.5">
                    <ArrowDownRight className="h-3.5 w-3.5 text-blue-400" />
                    Alle Abfahrten ({selectedRoute.downhill_profile?.length || 0})
                  </h3>
                  {activeDescentIndex != null && (
                    <button
                      type="button"
                      onClick={() => setActiveDescentIndex(null)}
                      className="text-[10px] text-accent hover:underline"
                    >
                      Karten-Fokus zurücksetzen
                    </button>
                ...
                {!selectedRoute.downhill_profile?.length ? (
                  <p className="text-xs text-text-muted italic bg-surface p-4 rounded-xl text-center">
                    Keine signifikanten Abfahrten auf dieser Route erkannt.
                  </p>
                ) : (
                  <div className="overflow-x-auto border border-border rounded-xl bg-surface">
                    <table className="w-full text-xs text-left">
                      <thead className="bg-surface-2 border-b border-border text-[10px] uppercase text-text-muted">
                        <tr>
                          ...
                          <th className="py-2.5 px-3">Distanz (km)</th>
                          <th className="py-2.5 px-3">Länge</th>
                          <th className="py-2.5 px-3">Höhenverlust</th>
                          <th className="py-2.5 px-3">Ø Gefälle</th>
                          <th className="py-2.5 px-3">Min Gefälle</th>
                          <th className="py-2.5 px-3">Kategorie</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-border/40">
                        {selectedRoute.downhill_profile.map((d: DescentSegment, i: number) => {
                          const isActive = activeDescentIndex === i;
                          return (
                            <tr
                              key={i}
                              onMouseEnter={() => {
                                setActiveDescentIndex(i);
                                setActiveClimbIndex(null);
                              }}
                              onClick={() => {
                                setActiveDescentIndex(i);
                              ...
                              className={`transition-colors cursor-pointer ${
                                isActive ? "bg-blue-500/15 font-semibold text-text" : "hover:bg-surface-2/70 text-text-muted"
                              }`}
                            >
                              <td className="py-2.5 px-3 text-text font-bold">{i + 1}</td>
                              <td className="py-2.5 px-3 tabular-nums">km {d.start_km} – {d.end_km}</td>
                              <td className="py-2.5 px-3 tabular-nums">{(d.length_m / 1000).toFixed(1)} km</td>
                              <td className="py-2.5 px-3 tabular-nums text-blue-400 font-medium">-{Math.round(d.elevation_loss_m)} hm</td>
                              <td className="py-2.5 px-3 tabular-nums font-bold text-text">Ø {d.avg_grade_pct}%</td>
                              <td className="py-2.5 px-3 tabular-nums">{d.min_grade_pct}%</td>
